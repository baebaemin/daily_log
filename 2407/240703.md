...
- SSR은 HTML 포맷으로 온다. = 초기 렌더링을 서버에서 처리하고 클라이언트에서 JS 코드를 추가해 상호작용을 추가.
- RSC는 RSC Payload라는 JSON 포맷으로 온다. = 서버에서 생성된 React 엘리먼트를 직렬화(RSC Payload)하여 클라이언트로 전송하고, 클라이언트에서 초기 렌더링 및 확장을 수행.
  - => 컴포넌트 소스 뿐 아니라 외부 라이브러리까지도 서버에서 이미 모두 실행된 후 직렬화된 JSON 형태로 전달되기 때문에 어떠한 bundle도 필요하지 않다
  - => => Next에서 SSR을 사용하더라도 CSR과 똑같은 크기의 bundle을 다운받아야 했는데, RSC를 도입하여 TTI 개선에 크게 기여할 수 있게 되었다.
...
- 13 : 렌더링시 RCC는 자동으로 직렬화될 때 placeholder로 대체됨
- 13 : getServerSideProps / getStaticProps 사용 X
