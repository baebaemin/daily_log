...
export default async function PageA({ searchParams }) {
  ...
  if (!queryParam) {
    redirect("/pag-b"); // 서버에서 실행 중단 -> 클라이언트에 HTML 보내지 않음
  }
...
- redirect 하는 순간 이외의 로직은 실행되지 않으니 useRouter().push()보다 좋은 것 아닌가? 라고 생각했지만 ...

  - 클라이언트 사이드에서는 사용 불가
  - 에러 처리 로직이 있다면 의도치 않게 catch될 수도 있다
  - UX 측면에서 특정 링크로 이동시키기 전에 alert 할 시간을 벌 수 없음

    ```javascript
    // 즉시 redirect
    if (!user) redirect("/login"); // 갑작스러운 이동

    // vs 중간에 alert하고 넘어가기
    if (!user) {
      toast.error("로그인이 필요합니다.");
      setTimeout(() => router.push("/logim", 1000));
    }
    ```
